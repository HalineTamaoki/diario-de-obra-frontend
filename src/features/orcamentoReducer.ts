...
export const orcamentoReducer = {
    ...
    selecionarOrcamento: (state: DetalhesObraState, action: PayloadAction<IdObraId>) => {
      state.itemsObra = state.itemsObra.map(item => 
    ...
    desselecionarOrcamento: (state: DetalhesObraState, action: PayloadAction<IdObraId>) => {
      state.itemsObra = state.itemsObra.map(item => 
        item.id === action.payload.idObra ?
          {...item, 
    ...
    deletarOrcamento: (state: DetalhesObraState, action: PayloadAction<IdObraId>) => {
      state.itemsObra = state.itemsObra.map(item => 
        item.id === action.payload.idObra ?
          {...item, 
