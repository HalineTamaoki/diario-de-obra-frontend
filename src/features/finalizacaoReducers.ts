...
export const finalizacaoReducers = {
    editarDataFinalizacao: (state: DetalhesObraState, action: PayloadAction<{data: string | undefined, idObra: number}>) => {
    ...
    editarComentarioFinalizacao: (state: DetalhesObraState, action: PayloadAction<{comentarios: string, idObra: number}>) => {
      state.itemsObra = state.itemsObra.map(item => 
        item.id === action.payload.idObra ?
          {
            ...item, 
