...
export const OrcamentoCard = (props: IdObraIdItem) => {
    ...
    const itemObra = selectItemObra(detalhesObra, idItem);
    ...
    return (
        <div className='px-2 pt-2 overflow-auto max-h-60'>
            ...
            <Link to={`/orcamento/${idObra}/${idItem}/novo`} id={`orcamento-card-novo-${idItem}`} className='text-(--blue)! text-sm w-full block underline! hover:text-(--blue-2)! mb-3'>
                Adicionar novo
            </Link>
            {resumoOrcamentos && resumoOrcamentos.length > 0 ? (
              <div className='grid gap-2'>
                {resumoOrcamentos.map((orcamento) => 
                  <OrcamentoResumoCard orcamento={orcamento} key={orcamento.id} idItem={idItem}/>)}
              </div>  
            ) : (
